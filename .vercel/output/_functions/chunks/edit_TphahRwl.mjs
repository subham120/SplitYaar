...
var $$Edit = createComponent(async ($$result, $$props, $$slots) => {
	...
	if (!code || !expenseId) return Astro.redirect("/");
	...
	if (!expense) return Astro.redirect(`/trip/${code}`);
  ...
  <script>(function(){${defineScriptVars({ code })}
    if (!localStorage.getItem(\`split_yatra_member_\${code}\`)) {
      window.location.replace(\`/trip/\${code}/join\`);
		...
		return renderTemplate`<button type="button"${addAttribute(cat.id, "data-category")}${addAttribute(`category-chip rounded-pill px-md py-xs text-[12px] font-sans transition-all select-none cursor-pointer border ${isActive ? "bg-primary text-on-primary border-primary font-semibold" : "bg-zinc-100 text-ink border-transparent hover:bg-zinc-200"}`, "class")}>${cat.label}</button>`;
	})}</div><input type="hidden" name="category" id="hidden-category"${addAttribute(expense.category, "value")}></div><!-- Split Type Segmented Control --><div><span class="block text-caption-strong text-ink font-semibold mb-xs">Split Method</span><div class="flex bg-zinc-100 rounded-pill p-[2px] border border-zinc-200 select-none"><button type="button" id="btn-split-all" data-type="equal_all"${addAttribute(`split-type-btn flex-1 text-center py-xs text-button-utility font-sans font-medium rounded-pill transition-all cursor-pointer ${expense.splitType === "equal_all" ? "bg-white text-ink shadow-sm" : "text-ink-muted-48 hover:text-ink"}`, "class")}>Split Equally</button><button type="button" id="btn-split-selected" data-type="equal_selected"${addAttribute(`split-type-btn flex-1 text-center py-xs text-button-utility font-sans font-medium rounded-pill transition-all cursor-pointer ${expense.splitType === "equal_selected" ? "bg-white text-ink shadow-sm" : "text-ink-muted-48 hover:text-ink"}`, "class")}>Selected</button><button type="button" id="btn-split-custom" data-type="custom"${addAttribute(`split-type-btn flex-1 text-center py-xs text-button-utility font-sans font-medium rounded-pill transition-all cursor-pointer ${expense.splitType === "custom" ? "bg-white text-ink shadow-sm" : "text-ink-muted-48 hover:text-ink"}`, "class")}>Custom (₹)</button></div><input type="hidden" name="splitType" id="hidden-split-type"${addAttribute(expense.splitType, "value")}></div><!-- Section C1: Equal (Selected) Exclusions Box --><div id="section-split-selected"${addAttribute(`${expense.splitType === "equal_selected" ? "" : "hidden"} bg-surface-pearl border border-hairline rounded-md p-md`, "class")}><span class="block text-caption-strong text-ink font-semibold mb-sm">Who is included in this split?</span><div class="grid grid-cols-2 gap-sm">${members.map((m) => {
		const isIncluded = expenseShares.some((s) => s.memberId === m.id);
		return renderTemplate`<label class="flex items-center gap-xs text-caption font-medium select-none cursor-pointer"><input type="checkbox"${addAttribute(m.id, "value")}${addAttribute(isIncluded, "checked")} class="exclusion-checkbox w-4 h-4 rounded-xs border-hairline text-primary focus:ring-primary cursor-pointer"><span class="truncate">${m.name}</span></label>`;
	})}</div></div><!-- Section C2: Custom Splits Input Fields --><div id="section-split-custom"${addAttribute(`${expense.splitType === "custom" ? "" : "hidden"} bg-surface-pearl border border-hairline rounded-md p-md space-y-sm`, "class")}><div class="flex items-center justify-between border-b border-divider-soft pb-xs mb-xs"><span class="text-caption-strong text-ink font-semibold">Allocate custom amounts</span><span class="text-[12px] font-semibold text-ink-muted-48" id="custom-total-counter">Allocated: ₹0.00 / ₹0.00</span></div><div class="space-y-xs">${members.map((m) => {
		const share = expenseShares.find((s) => s.memberId === m.id);
		const shareVal = share ? (share.sharePaise / 100).toFixed(2) : "";
		return renderTemplate`<div class="flex items-center justify-between gap-md"><span class="text-caption font-medium text-ink truncate max-w-[150px]">${m.name}</span><div class="relative max-w-[120px] w-full"><span class="absolute left-xs top-1/2 -translate-y-1/2 text-caption text-ink-muted-48">₹</span><input type="number" step="0.01" min="0"${addAttribute(shareVal, "value")} placeholder="0.00"${addAttribute(m.id, "data-member-id")} class="custom-member-input w-full bg-white text-ink text-right font-sans text-caption border border-hairline rounded-pill pl-md pr-xs py-[6px] focus:outline-none focus:border-primary placeholder:text-zinc-200"></div></div>`;
	})}</div><!-- Allocation error text --><p id="custom-split-error" class="hidden text-fine-print text-red-600 font-semibold"></p></div><!-- Save Button --><button type="submit" id="submit-btn" class="w-full bg-primary text-on-primary font-sans font-medium text-body py-[12px] rounded-pill hover:bg-primary-focus active:scale-95 transition-all select-none cursor-pointer mt-lg text-center">Save Changes</button><!-- Delete Expense Button --><button type="button" id="delete-btn" class="w-full bg-transparent text-red-500 border border-red-500 font-sans font-medium text-body py-[12px] rounded-pill hover:bg-red-50 active:scale-95 transition-all select-none cursor-pointer text-center block mt-xs">Delete Expense</button><!-- Error Message --><p id="error-msg" class="hidden text-caption text-red-600 bg-red-50 border border-red-200 rounded-sm p-sm mt-xs"></p></form></div></div>
...
}, "D:/project/SplitYaar/src/pages/trip/[code]/expense/[expenseId]/edit.astro", void 0);
var $$file = "D:/project/SplitYaar/src/pages/trip/[code]/expense/[expenseId]/edit.astro";
var $$url = "/trip/[code]/expense/[expenseId]/edit";
//#endregion
//#region \0virtual:astro:page:src/pages/trip/[code]/expense/[expenseId]/edit@_@astro
